...
import time
# django won't let you upload files to an absolute path we can get around this
# by using djangos FileSystemStorage object in our upload fields
from django.core.files.storage import FileSystemStorage

dcm2bids_file_location = FileSystemStorage(location=settings.DCM2BIDS_FILES)
...
class Subject(models.Model):
    ...
    dicoms. This id will be determined from the information gleaned in pydicom
...
class DcmToBidsJson(models.Model):
    """
    This model holds the path and the text of the dcm2bids conversion json
    that is used to convert dicoms to bids format.

    : name : provides the user with an easily referenced name
    : session : ties  this configuration file to a session, since bids is
    for the most part universal one Json can be tied to many sessions/subjects
    : path : this is the path to the json file
    : dcm_to_bids_txt : this is the content of the the json stored as a text
    object in the database. I'm not sure of the utility of doing this, but
    since it's such small thing I don't see the disadvantages of storing it
    either.
    """
    name = models.CharField(max_length=255)
    sessions = models.ManyToManyField("Session", related_query_name='json', blank=True)
    path = models.TextField(blank=True, null=True)
    dcm_to_bids_txt = models.TextField(blank=True, null=True)
    dcm_to_bids_file = models.FileField(null=True, blank=True, upload_to=dcm2bids_file_location)
...
class HasBeenIndexed(models.Model):
    date_indexed = models.DateTimeField(null=False)
    directories_indexed = models.TextField(null=False)


class ProgressIndicator(models.Model):
    time_started = models.FloatField(default=time.time())
    conversion_output_path = models.TextField(blank=True, null=True)
